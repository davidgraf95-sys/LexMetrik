...
// Beweist, dass die neuen Kopf-Interaktionen (A4 «Ansicht»-Dropdown öffnen +
// Switches togglen, A3 Breadcrumb-Klick) auch gedrosselt ohne spürbaren Lag
...
// 2-Spalten-Kontextkopf + Breadcrumbs) → deckt alle drei A-Punkte ab.
...
test('A9: «Ansicht»-Dropdown + Breadcrumb-Klick flüssig unter CPU-Throttle, CLS 0', async ({ page }) => {
  ...
  // Dropdown schliessen (Escape), dann A3: Breadcrumb-Klick springt flüssig.
  await page.keyboard.press('Escape');
  await expect(gruppe).toBeHidden();
  const glied = page.locator('[data-kontext-kopf] nav[aria-label="Standort im Erlass"]').getByRole('button').first();
  await expect(glied).toBeVisible();
  t0 = Date.now();
  await glied.click();
  await page.waitForTimeout(600);
  expect(Date.now() - t0, 'Breadcrumb-Sprung zu langsam').toBeLessThan(5000);
