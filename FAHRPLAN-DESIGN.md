...
Abnahmegrundlage für David.~~ **ERLEDIGT 10.6.2026 abends:** axe-Stichprobe
als CI-Spec (`e2e/a11y.e2e.ts`: 1 critical + 2 serious gefixt, 2 serious als
Entscheid David offen) + Tastatur-Spec (`e2e/tastatur.e2e.ts`) +
Screenshot-Serien 3 Stände × 6 Motive × 3 Breiten — alles in
`abnahme/design-2026-06/BERICHT.md`. OFFEN bleibt: Davids Abnahme
(Entscheid-Block E im Bericht: B-1 Kalender-Kontrast, B-2 Link-
Unterstreichung) + manuelle Screenreader-Posten C.1–C.3.
(Checkboxen unten 10.6. nachgeführt — Etappen waren seit 7.6. gebaut, nur
nie abgehakt; 2.5/2.7/4.1/4.3/4.4 = «kein Handlungsbedarf/widerlegt» laut
Abweichungsliste oben, nicht gebaut.)
...
- [x] 1.1 **`src/App.css` löschen**: wird NIRGENDS importiert (verifiziert:
...
- [x] 1.2 Kurzer Grep-Nachweis im Commit, dass keine Klasse aus App.css in
  TSX vorkommt.
...
- [x] 2.1 **Neue Klasse `.lc-highlight`** (Messing-Hervorhebungsbox) in
...
- [x] 2.2 **Fehler-Boxen vereinheitlichen**: `FehlerBox`
...
- [x] 2.3 **Karten-Nachbauten**: `border border-line rounded-lg p-3/4
  ...
  Freihand-Klassen.
- [x] 2.4 **Lösch-/Entfernen-Textbuttons** standardisieren
...
- [x] 2.5 **EckdatenKachel-Rollout**: in 7 Dateien genutzt; Teuerung-/
  Verzugszins-/Streitwert-Form bauen Kacheln mit `.lc-tile` frei nach →
  vereinheitlichen.
- [x] 2.6 **NormChip (ErgebnisAnzeige.tsx) vs. NormLink (vorlagen/ui.tsx)**:
...
- [x] 4.2 **Timelines** (VerzugszinsTimeline:96/138, KuendigungTimeline:88–97):
...
- [x] 5.1 **Pro-Kopf aufwerten**: h1 statt h2 + `scale-rule` wie die
  Startseite — Pro ist die Hauptarbeitsfläche der Zahlkundschaft und
  wirkt derzeit hierarchisch untergeordnet.
- [x] 5.2 **Sektions-Rhythmus statischer Seiten** (Methodik, Über,
...
- [x] 5.3 **Suspense-Fallback** (App.tsx ~80): neutraler «Wird geladen …»-Text
  → kleine `scale-rule` + ruhige Zeile, damit Laden nicht wie ein Fehler
  aussieht.
- [x] 5.4 **NotFound** in die Familie holen: Overline + `scale-rule` +
  Standard-Seitencontainer.
- [x] 5.5 **`.lc-list`** einführen (Messing-Gedankenstrich als Marker) für
