...
## STAND 27.6.2026 — VOLL UMGESETZT + auf `main` (deployt; Code-verifiziert 28.6.)
- **Schritt 1 (P1a/P1b/P1c) ERLEDIGT:** Klassifizierung `leit = istBge || !!bge_reference`
  (`adapter-entscheide.ts:374`), `enumeriereBge`/`holeBgeLeitentscheid` (:511/:562),
  CLI `--bge-von`/`--bge-limit` (`normtext-entscheide.ts:43f`). Korpus: 272 Leitentscheide,
  533 mit `bgeReferenz` (B2-§8-Bug behoben). Harmonisierte Leitansicht: eigene Browse-Sektion
  + `nurBge`-Filter (`browse.ts:289/215`). Commit `e1ffb194`, Filter-Merge `e2bc24ce`.
- **Schritt 2 (P2) ERLEDIGT:** `azaUrteil` (`typen.ts:77`) + BGE-Umschalter voll/auszug
  (`EntscheidLeser.tsx:100/178/188`), ehrlicher Fallback bei fehlendem aza-Urteil (:192).
  Commit `f8146c2e`.
- **OFFEN:** nur Davids fachliche Abnahme (Zeitsperre bis 1.12., alles `kuratierung:'maschinell'`);
  36 % BGE ohne Volltext-Auflösung bewusst (§5). **Kein offener Code-Punkt** → ins `archiv/`.
