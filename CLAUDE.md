...
automatisch auf Prod aus (Projekt `lexmetrik`, lexmetrik.vercel.app) — damit IST
die Freigabe zum **Merge nach `main`** die
...
2. **Plan-Stand abfragen → Abgleich → Bündeln → Einordnen.** **Vor** dem Start einer Bau-Einheit den aktuellen Plan lesen, nicht nur den eigenen Auftrag: `npm run plan:next` (oberster offener Schritt · `dep`/Blocker · was schon `wip` ist ⇒ Doppelarbeit) + den einschlägigen `npm run fahrplan`-Slice. **Erledigtes danach abhaken** — der Plan wird in BEIDE Richtungen gepflegt, sonst verliert er die Steuerungswirkung (20.7.2026: zehn Schritte gleichzeitig `wip`, mehrere längst erledigt; zwei Sessions fixten parallel denselben Defekt in `SuchResultate.tsx`). Vor dem Einsortieren prüfen, ob ein verwandter/überlappender Schritt existiert oder dieselben Dateien / dasselbe Subsystem / dasselbe Datenasset / dieselbe Prüf-Fläche berührt sind → zu **einer** Bau-Einheit bündeln (einmal bauen, prüfen, deployen). **Nicht über-bündeln:** keine Risiko-Klassen mischen (Rechtsinhalt ≠ reines UI in einer Einheit, §1/§3), die Einheit klein genug für ein sauberes Gate und golden byte-gleich (§6). Kohärent mit «nie zwei 26×-Assets parallel» (ROADMAP-Leitprinzip 4) und §12 (Worktree-Isolation bei Datei-Kollision).
