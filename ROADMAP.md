...
- [ ] **Davids Alltags-Fehlerfunde** *(`W2·18-FEHLERBUCH`, stehender Sammel-Schritt, Entscheid David 8.8.2026)*
  ...
  **Detail:** [FAHRPLAN-OFFENE-BEFUNDE.md](fahrplaene/FAHRPLAN-OFFENE-BEFUNDE.md) §4 — dort die
  ...
  - [ ] **Wächter «rectifies-Ziel vs. Berichtigungstext»** *(Gegenprüfung PR #827, 12.9.2026)* — Fedlex' `jolux:rectifies` kann auf das falsche AS-Dokument zeigen (AS 2025 686/SKV: Ziel AS 2025 648 = TAFV 2, im amtlichen Text steht AS 2025 644 = SKV; AS 2024 144/SSV: Sammelberichtigung mit nur einem `rectifies`). Tor: die im Berichtigungstext (Filestore-HTML, «(AS … ; SR …)») genannte AS-Fundstelle gegen das `rectifies`-Ziel messen, Abweichung als Befund listen statt in Prosa übersetzen (§7/§17; heute nur Docstring-Vermerk in `scripts/normtext/revisionen-generieren.ts`).
  - [ ] **`check:schlankheit` rot auf main: `scripts/normtext/adapter-lexwork.ts` 926 Z. (Baseline 839)** *(Nullprobe zweier Fixer 12.9.2026 auf 6aca2901e)* — §6.6-Split in Leaf-Module, Baseline nicht anheben; Risikopfad ⇒ Golden byte-gleich + Gegenprüfung (kein Jules-Ticket). Zusätzlich seit PR #827 (12.9.2026) `src/components/kontext/KontextPanel.tsx` 813 Z. (Schwelle 800) — reine UI-Komponente, kein Risikopfad; Split ohne Gegenprüfungspflicht.
  - [ ] **§6.7-Kennzahl-Regel fehlt für Wächter mit UND-Bedingungen** *(Gegenprüfung PR #828 Runde D1, 12.9.2026, gehört ins Fehlerbuch-Dach `W2·18-FEHLERBUCH`)* — der Vollabdeckungs-Wächter im `--nur`-Pfad (Kanton-Fremd-Drift, PR #828 noch offen) mass durch eine UND-verknüpfte Bedingung still 0 statt der erwarteten 267 `erlassNr`-Verlust-Kandidaten (Mehrheits-Heuristik statt 1:1-Übernahme). §6.7 verlangt «einmal rot, einmal grün» für neue Tore — bei UND-verknüpften Bedingungen genügt das nicht, wenn keine Teilbedingung je einzeln geprüft wird: die Ist-Zahl (nicht nur 0/&gt;0) gehört einmal in den Testbeweis. Gehört inhaltlich in Skill `refactoring` §6.7 (ausserhalb der Whitelist der Session, die diese Zeile anlegte); Verankerung dort als eigener Schritt.
  - [ ] **Falscher Freund «BMV» im Kanton-Pfad** *(Fixer #823, 12.9.2026)* — `kanton/SG-3849` meint mit «BMV» die Schutzbautenverordnung, der Normtext-Link zeigt auf die Berufsmaturitätsverordnung (Bund); Kürzel-Auflösung im Kanton-Pfad muss kantonale Kürzel vor Bund-Kürzeln prüfen oder benannt ausschliessen (§7).
...
- [x] **Drift-Nachverifikation der Tarif-Stammdaten (34 Erlasse, 93 Einträge)** *(`W3-TARIF-NACHVERIFIKATION`, Auftrag David 6.9.2026, Befund Tor-Erstlauf `check:tarif-drift`)*
  ...
  - [ ] **Werkzeug-Falle Norm-PDFs** *(6.9.2026, gehört ins Fehlerbuch-Dach `W2·18-FEHLERBUCH`)* — `Read` auf PDF > 200 KB blockt `lese-schutz.py`, `pdftotext` fehlt auf dieser Maschine; funktionierender Ersatz: PyMuPDF (`python3 -c "import fitz; …"`).
