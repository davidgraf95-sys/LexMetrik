          ...
          {NAV.map((n) => {
            ...
            return (
              <Link key={n.to} to={n.to}
                className={`px-3 py-2 rounded-md text-body-s font-medium no-underline transition-colors ${
                  aktiv ? 'text-brass-700 bg-brass-100' : 'text-ink-600 hover:text-ink-900'
                }`}>
                {n.label}
              </Link>
            );
