...
- [x] **Nebenläufigkeit** (`p-limit`, Concurrency 4–8) in den Fetch-Schleifen (`scripts/normtext-snapshot.ts`, `check-drift.ts --netz`) — I/O-bound, Faktor ~Concurrency; Merge bleibt `sort()` (deterministisch). → FAHRPLAN-DATENHALTUNG §6 **✅ 3.7.2026 (Concurrency 4, höflich): Kanton-/HTM-/ZH-/PDF-Loops + 4 check-drift-netz-Loops; Bund-Loop UNBERÜHRT (DB-Flip). Muster «parallele pure Fetches → serielle Verarbeitung in Inventar-Reihenfolge». Doppelbeweis: byte-identisch seriell↔parallel (UR+SO 566 + ZH 198 Snapshots, live) + adversariale Opus-Gegenprüfung bestanden.**
