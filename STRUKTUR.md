...
- **Baustelle A2 · TBT je Job normieren — mit Wirkungsnachweis.** Eine synthetische, deterministische CPU-Last (`dist/_perf-kalibrier.html`, zur Laufzeit geschrieben, kein App-Code, kein Netz) wird über dieselbe Lighthouse-Kette gemessen; `TBT normiert = TBT roh / (TBT kalibriert / 1120)`. **Belegt an einer frischen 8er-Matrix-Messreihe** (neuer Workflow `perf-kalibrierung.yml` = 8 unabhängige Runner-Zuteilungen in EINEM Lauf, je Median aus 3; Lauf 29765490018): Kalibrier-Referenz streut 720…1476 ms (**Faktor 2.05** — der Runner-Pool, wie vermutet). OR-TBT **roh** Mittel 4185 · sd 1307 · **CV 31.2 %** → **normiert** Mittel 4231 · sd 698 · **CV 16.5 %**; Korrelation zur Runner-Geschwindigkeit **r +0.83 → −0.21**. Die Streuung ist also fast halbiert und der Runner-Einfluss herausgerechnet — dieselben Messpunkte, gepaart ausgewertet.
- **Schwellen wieder SCHARF gestellt** (die 27-Lauf-Historie des alten Regimes wurde als entwertet verworfen, nicht übernommen): **OR-TBT normiert 5900 ms** ⇒ ~**0.8 %** Rot-durch-Rauschen bei **39 %** Abstand zum Ist — gegen den bisherigen Roh-Deckel 6500 (3.8 % / 55 %) auf BEIDEN Achsen besser. **Startseite normiert 300 ms** statt roh 1500: Kopffreiheit **651 % → 48 %**, dort war der alte Deckel reine Dekoration. Nachgezogen: OR TTI 15000→13000, Start LCP 11000→10000 (dort sd nur 36 ms über alle 8 Runner!), Start Score 40→55. Die Roh-Deckel bleiben als **weites Sicherheitsnetz** (OR 9000), falls die Kalibrierung ausfällt — Plausibilitätsband + ehrliche Meldung statt stillem Durchwinken (§8). **Damit ist der Übergangszustand beendet, in dem CLS die Regressions-Last allein trug.**
...
- **Beweis:** voller `npm run gate` **GRÜN** · `check:e2e-shards` grün (47 Specs, Union deckungsgleich) · `check:perf-lighthouse` lokal **GRÜN** gegen die neuen, engeren Deckel · volle e2e-Fläche lokal grün · `find src -name '__*'` leer. Kein Produktionscode berührt (Diff: Prüflogik, Test-Instrumentierung, CI-Konfiguration) ⇒ Inhalts-/Rechtsregel-/Funktions-Treue per Definition unberührt. **Gegenprüfung n/a** — reine Prüflogik.
- **Lektion:** ein **bitgenau reproduzierbarer** Fehlwert ist nie Rauschen — er ist ein Zeiger auf ein deterministisches Artefakt, meist im Messaufbau selbst. Und: bevor man Schwellen lockert, prüfe, ob das Instrument das richtige Fenster misst. Von 18 Fehlläufen waren am Ende **13 Mess-/Budget-Artefakte**, 4 bereits behoben und **einer** ein echtes Produktsignal.
