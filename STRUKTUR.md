...
- **Fix — Schärfe wandert dorthin, wo Signal ist, statt pauschal zu lockern:** (a) **TBT/OR 4000 → 4800 ms**, kalibriert gegen genau diese Verteilung: 4800 liegt bei z ≈ +1.8 ⇒ erwartete Rot-durch-Rauschen-Quote fällt von ~26 % auf ~3.5 % (main-Pushes ~30 % → ~12 %); der Deckel bleibt **unter** dem beobachteten Maximum (5094 ms), ist also gelockert, nicht ausgeschaltet, und fängt weiterhin jede echte Regression ab ~+1200 ms · (b) **Gegengewicht/Verschärfung: CLS/OR 0.15 → 0.05, Start 0.10 → 0.05** — CLS ist laut §15.2 «der eigentliche Regressions-Fänger», lag über dieselben 27 Läufe aber nur bei **0.004–0.008** (Deckel = 19–37× Ist, fing faktisch nichts); 0.05 ist ~6× über dem beobachteten Maximum und deckt sich mit dem Budget der CLS-e2e-Specs · (c) die Einzelwerte je Lauf werden neu mitgedruckt, damit die nächste Session nicht wieder über EINE Zahl ohne Streuung diskutiert (§8). Die vollständige Beleglage steht als Kommentarblock **im Tor selbst**, nicht nur im PR.
- **Verworfener Zwischenstand — der Messaufbau selbst war eine Störgrösse.** Erster Anlauf hob zusätzlich `PERF_RUNS` in CI von 3 auf 5 (mehr Mittelung gegen die Streuung). Der CI-Lauf widerlegte das: das Tor teilt **EINE Chrome-Instanz über alle Läufe BEIDER Messseiten**, die Instanz driftet, und die zuletzt gemessene Seite erbt die Drift — die Startseite sprang von historisch 143–237 ms TBT auf **1543 ms** (rot) und OR-LCP von ~3.5 s auf 11.3 s, **ohne jede App-Code-Änderung**. Mehr Läufe beruhigen hier also nicht, sie verschlechtern die späteren Werte. `RUNS` daher wieder auf 3; Chrome-Isolation je Lauf ist der richtige Weg, ändert aber das **Messregime** (absolute Werte verschieben sich, die 27-Lauf-Historie wird entwertet) ⇒ bewusst **nicht** mitgebündelt (§14.2), sondern als eigener Schritt mit Neukalibrierung in ROADMAP/QS-PERF notiert.
