...
## Session 16.7.2026 — Suchgüte-Eval-Harness (advisory Mess-Werkzeug, W2·10-UI-NAV, Worktree `lm-eval`, Branch `feat/suche-eval-harness`)

**Auftrag (David-Go «go eval-harness» + ultracode).** Ein deterministisches, LLM-freies Mess-Werkzeug für die RANKING-GÜTE der ECHTEN Produkt-Suche — `scripts/suche-eval.ts` (`npm run eval:suche`) gegen ein committetes, zweifach verifiziertes Gold-Set `scripts/suche-eval-gold.json` (69 Paare, 4 Klassen: normzitat/umgangssprache/bge/stichwort; Belege je Paar gegen `struktur/bund/*.json` bzw. `rechtsprechung/register.json`). **Kein Parallel-Index (§5):** der Harness ruft die Produktions-Funktionen (`parseNormQuery`/`sprungGruppe`, `parseBgeSprung`/`bgeSprungGruppe`, `baueSuchFn`=FlexSearch-Recall + `artikelRanking.rangiere`, `gesetzGruppe`/`artikelGruppe`/`entscheidGruppe`) und assembliert sie in der Hook-Reihenfolge; der Bund-Index wird FRISCH aus den Snapshots gebaut (`baueBundIndex`, MIT m/n/g), nicht die veraltete committete JSON. Rang wird IM Produkt-Kanal des Inhaltstyps gemessen (Sprung⧺Gesetzestext / BGE-Sprung⧺Rechtsprechung / Sprung⧺Gesetze); gruppenübergreifende Reihenfolge (UI/A6) bewusst nicht mit Kanal-Ranking vermischt. Metriken Recall@1/5/10·MRR·NDCG@10 je Klasse. **Baseline 16.7. erhoben** (`bibliothek/werkzeuge/suche-eval-baseline-2026-07-16.md` + INDEX-Zeile): normzitat/bge 0.83 (Rang-1-Zitate stark), stichwort Recall@5 0.75, **umgangssprache Recall@10 0.18 = grösste Lücke** (Kompositum ≠ FlexSearch-forward-Präfix, empirisch bestätigt: «Verjährungsfrist…» findet OR 127 nicht, «Werkeigentümerhaftung» 0 Treffer); Nebenfund FR/SR-Alias im Norm-Sprung unvollständig (Cst→BV, LDIP→IPRG fehlen in `normQuery.ALIAS`). Determinismus verifiziert. **Advisory: KEIN Gate, KEIN CI-Einbau** (vermerkt im Skript-Kopf). §14: additiver Einzeiler im ROADMAP-Schritt W2·10-UI-NAV. **Tore:** `npm run gate` grün ausser **check:materialien** (pre-existing date-rot: VERN-2025-124/-58 `fristEnde 2026-07-15 < heute`, unverändert auf HEAD — nicht dieser Diff). Gegenprüfung n/a — internes Mess-Werkzeug, read-only, kein Rechnen/Extraktion/Norm-Tarif. Trailer `Roadmap: W2·10-UI-NAV`.
