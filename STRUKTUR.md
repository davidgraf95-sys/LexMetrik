...
- **Fix — Schärfe wandert dorthin, wo Signal ist, statt pauschal zu lockern:** (a) `PERF_RUNS` in CI **3 → 5** (Median aus 5 hat 0.53·σ statt 0.70·σ ⇒ Streuung des berichteten Werts ~687 → ~525 ms; Kosten ~90 s im eigenen Job) · (b) **TBT/OR 4000 → 4800 ms**, kalibriert auf Mittel + ~2 sd des beruhigten Medians (3555 + 2·525 ≈ 4605), fängt weiterhin jede echte Regression ab ~+1200 ms · (c) **Gegengewicht/Verschärfung: CLS/OR 0.15 → 0.05, Start 0.10 → 0.05** — CLS ist laut §15.2 «der eigentliche Regressions-Fänger», lag über dieselben 27 Läufe aber nur bei **0.004–0.008** (Deckel = 19–37× Ist, fing faktisch nichts); 0.05 ist ~6× über dem beobachteten Maximum und deckt sich mit dem Budget der CLS-e2e-Specs. Die vollständige Beleglage steht als Kommentarblock **im Tor selbst**, nicht nur im PR.
