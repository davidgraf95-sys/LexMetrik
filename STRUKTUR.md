...
## Session 24.6.2026 — TEIL B: entscheidsuche-Live-Suche + Sachverhalt-Gliederung (ultracode, Branch `entscheidsuche-livesuche`, NICHT deployt)

Auftrag David: entscheidsuche in LexMetrik nutzbar machen (Teil B des BGE-Fahrplans) + Sachverhalt der BGE „richtig aufteilen". Isolierter Worktree (§12), auf dem deployten Stand `eaff07c` aufgebaut. Gate grün (golden byte-gleich), adversarialer Bug-Check (1 MINOR + 1 MAJOR gefunden + gefixt), Playwright hell/dunkel/desktop/mobil 0 Konsolenfehler. **Kein Deploy** (Davids Ja offen, §9).
- **B2 Live-Volltextsuche** (`lib/rechtsprechung/livesuche.ts` + `components/rechtsprechung/LiveSuche.tsx` in `/rechtsprechung`): opt-in Discovery über den GANZEN CH-Korpus. **§7 empirisch verifiziert:** MCP `/mcp` blockt Browser-Origin (403) → direkter Browser-Fetch gegen `entscheidsuche.ch/_search.php` (CORS `*`). KEINE Engine (§2): externe, nicht geprüfte Treffer klar markiert, amtlicher Link je Treffer, Suchbegriff verlässt die App erst auf Klick (Berufsgeheimnis, §8). Reine `baueAnfrage`/`mappeTreffer` (lädt `attachment.content` nicht). Live-Call end-to-end verifiziert (13/20 echte Treffer).
- **Sachverhalt-Gliederung** (`lib/rechtsprechung/sachverhalt.ts`): BGE-Sachverhalt war ein Monolith mit Seiten-Rauschen. **§1-Befund:** Voll-Top-Marker-Erkennung (A./B./C.) ist UNSICHER (kollidiert mit Namen „B. Pte Ltd"). Sicher: nur an **Sub-Markern** (`Gross.Klein` A.a/B.b, sequenzvalidiert) trennen — über 258 BGE 0 Fehl-Splits; sonst EIN bereinigter Block (Fallback). **130 BGE gegliedert**, Seiten-Rauschen („BGE … S. NN") entfernt, wortinvariant-modulo-Rauschen (0 Drift), Offline-Re-Derive idempotent. **Bug-Check MAJOR gefixt:** Blind-Trim löschte Schluss-Namen (9 Fälle „…im E.") → nachlaufende Top-Marker jetzt nur bei passendem Folge-Top abgetrennt (Quell-Fix, wortinvariant). Adapter wired (künftige Importe). EntscheidBody zeigt Buchstaben-Label je Absatz.
- **Offen:** Davids Abnahme + Deploy-Entscheid (§9). B3 (Breiten-Ingestion) / B4 (Norm-Verzahnung live) / B5 (News) bleiben dokumentiert.
