...
## Session 16.7.2026 — QS-OPT-Fortsetzungskette (O-2 · O-1.9 · O-4/O-3.2 dispatcht) — Worktrees `lm-o2`, `lm-o19`, `lm-o2docs`

**Auftrag (FAHRPLAN-OPTIMIERUNG-2026-07, Fortsetzung der O-1/O-3.1-Kette; David-Freigaben 16.7.2026 per Auswahl-Dialog: O-2-Rahmen-Gate «voll freigeben», O-4-klein «FR/IT aufnehmen», O-1.9 «bauen».)**

- **O-2 Frische-Automatik → PR #259.** `scripts/fedlex-repin-batch.ts` (O-2.1): hebt überholte Bund-Pins in EINEM Lauf auf die geltende Fedlex-Konsolidierung (neues kons-Datum + kanonisches html-N via isExemplifiedBy, byte-präzise cache.sh-Feld-3/4-Ersetzung); **§7-treu — künftige Fassungen (der 1.8.-Berg, 10 Erlasse) werden NIE re-pinnt.** Komponiert `erhebe()` + `loeseHtmlManifeste()`; reine `erhebeRepin` (injizierbare fetchImpl) getrennt vom Schreiben; 5 Unit-Tests (byte-Präzision inkl. Komma-Anker + Throw-bei-No-Match). Zwei EIGENE Workflow-Dateien statt normen-monitor.yml-Eingriff (Kollision mit #244 vermieden): `fedlex-frische.yml` (scheduled Reparatur-Arm → Auto-Merge-PR + Handschritt-Zettel) und `turso-sync.yml` (O-2.6, Daten-Diff→Turso-Sync + Live-Parität). **`TURSO_AUTH_TOKEN` als CI-Secret gesetzt** (David-Rahmen freigegeben). Guard `FEDLEX_NUR_IMPORT` im Wiedervorlage-Generator (Import ohne Datei-Nebeneffekt; CLI verhaltensneutral). **Live-Befund (dry-run, unabh. verifiziert):** asylv2 fällig (2026-06-12→2026-07-14/html-1), chemrrv überholt aber html-N noch nicht publiziert (Handschritt) — **Korpus in diesem PR NICHT mutiert** (§14.2 Risikoklassen getrennt; die scheduled Automatik repariert). **Gegenprüfung bestanden** (unabh. Opus gegen Fedlex-SPARQL/Filestore: asylv2/html-1 verifiziert, §7-Berg USG/KVV/AVIV/BVV2/KLV echt künftig korrekt ausgeschlossen, chemrrv 0-Manifestation korrekt geflaggt, byte-präzise Ersetzung, Guard-neutral). tsc/golden(209)/20 Fedlex-Tests/eslint/YAML grün. Trailer `Roadmap: QS-OPT`.
- **O-1.9 Minimal-Fehlerkanal → PR #257.** Edge-`api/fehler.ts` ('self', POST-only, ratenbegrenzt pro Isolate, sanitisiert = Log-Injection-Schutz, nur meldung/route/build — **keine Eingaben, kein Fingerprinting, keine IP/Header**) + gesampelter fire-and-forget-Client `components/fehlermeldung.ts` (nur pathname, wirft nie, keepalive) + ErrorBoundary/window.onerror/unhandledrejection-Verdrahtung + `VITE_BUILD_ID`. Datenschutz-Absatz als **ENTWURF** markiert (§8, David-Wortlaut bei Abnahme). 5 Unit-Tests. tsc/golden/eslint/vite-build grün; kein Risikopfad → Gegenprüfung n/a. Trailer `Roadmap: QS-OPT`.
- **Dispatcht (eigene Bau-Agenten/Worktrees):** O-4 Alias-Tabelle FR/IT (`feat/qsopt-o4-alias`, Risikopfad-Gegenprüfung) · O-3.2 Flake-Wurzel (separat fortgesetzt). **Offen/Handoff:** O-4 DE-Filter-Heben + Sprach-Badge liegt als isolierter Worktree-WIP `feat/qsopt-o4-frit` (Code fertig, **schwere Korpus-Regenerierung + Gegenprüfung ×3 Sprachen offen**); O-4-Chips ans Ketten-Ende (Reader-PR-Kollision); O-3.3 Sharding wartet auf Hotfix #248-Merge; O-3.4 e2e-Masse offen.
- **Abhängigkeit:** grünes CI aller Ketten-PRs setzt Hotfix **#248** (bibliothek-check S7.3) auf main voraus — bis dahin `check:bibliothek` vorbestehend rot (nicht diese Diffs). Auto-Merge auf #257/#259 armiert.
