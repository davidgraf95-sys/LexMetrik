...
## Session 3.7.2026 — Fedlex-Portfolio Paket 1: Gesetze-Currency (QS-CURRENCY, P0), Branch `feat/fedlex-paket1-currency`

**Gebaut (David-Freigabe 3.7.; Risiko-Pfad Extraktion/Pins; FAHRPLAN-FEDLEX-PORTFOLIO §Paket 1, P1-a+P1-b):** **(1) Monitoring-Härtung P1-b:** Pin-Parser-Regex `fedlex-pins.ts` `([a-z_]+)`→`([a-z0-9_]+)` — die «11 Volltexte ohne Pin» des Gap-Reports waren in Wahrheit **parser-blinde Ziffern-Pins** (asylv1-3, argv1-5, bvv_2, bvv3, co2_gesetz): 207→**218 überwachte Pins**; Selbsttest `src/tests/fedlex-pins.test.ts` (Parser==Roh-Zeilen); **Pin-Coverage-Assertion** neu in `check:normtext` (jeder Bund-`snapshot`-Erlass braucht einen cache.sh-Pin, negativ rot-getestet); **`check:pdf-netz`-Currency-Fix**: alte notation-Join-Query × `LIMIT 300` = Partial-Result-Falle (kartesisches Produkt schnitt die neuesten Daten ab — der 21 Jahre alte EMRK-Pin bestand fälschlich GRÜN) → jetzt ELI-Abstract-Query wie `fedlex-versionen-pruefen.ts`; Gegenprüfungs-Glob `istRisikoPfad()` + Tests um `scripts/fedlex-*` erweitert. **(2) Datenlauf P1-a:** frischer Ist-Befund 3.7. per SPARQL == Gap-Report; **18 stale Snapshots** (kvg/kvv/klv/svg/vrv/ssv/vts/rpg/rpv/bpv/vil/fdv/mepv → 20260701 · argv2 → 20260201 · asylv1-3/icao → 20260612) **+ 2 PDF-Embeds** (EMRK → 20220916 mit kanonischem pdf-a-Suffix `-2` [neues Feld `pdfN`; suffixlose URL liefert 200 mit ÄLTEREM Re-Issue!], NYÜ → 20260506) auf die geltende Konsolidierung; **html-N je Erlass via `jolux:isExemplifiedBy` SPARQL-kanonisch** (klv=8/vrv=8/**ssv=14** — ausserhalb des cache.sh-Fallbacks -1..-5, N-Feld load-bearing); Artikel-Diff je Erlass alt→neu **ohne Verlust** (+81 neue Artikel per Juli-Revisionen; eId-Reshuffles asylv2 disp_N→disp_uN/Merge-Schreibweisen, svg disp_u2/art_108→art_108, vil art_27_bbis, vrv/rpv/asylv3 annex-Renames — Text 1:1 erhalten); SSV-Bildsatz erneuert (verwaiste Alt-PNGs raus); AsylV2 art_41 trägt Fedlex-QUELLseitig `[tab]`-Artefakt → `ARTEFAKT_ERWARTET` (24 Stellen/14 Artikel); KVG art_7/62/64a + KVV art_94/99/100 absatzweise diff-verifiziert (materiell identisch, Engine unberührt); Quellen-Register nachgeführt.

**Verifikation + Tore:** Update-Tor «nur Ziel geändert» (18/218 echte Diffs, 200 Datum-Churn zurückgesetzt) · 54/54 Wortlaut-Stichproben (3 Artikel je Erlass: Anfang/geänderter Bereich/Ende, Containment gegen kanonisches Quell-HTML) · `check:fedlex-versionen` **GRÜN 218 Pins/0 stale/56 künftig** · `check:pdf(-netz)` · `check:normtext` inkl. Coverage · tabellen/invarianten/struktur-konsistenz/vollstaendigkeit/bilder/suchindex · `check:paritaet` 2953 + `check:datenhaltung` (Manifest regen.) · `golden:vergleich` **201 byte-gleich** · tsc/test (3075+)/lint/build · `check:plan` (QS-CURRENCY im Inventar). **Gegenprüfung: 3+1 unabhängige Opus-Agents** (frischer Kontext, gegen die amtliche Quelle; Verdikt im Gegenprüfungs-Register). **F2-Folgefix aus der Gegenprüfung:** Extraktor-Ordinal-Vokabular auf sexies…decies erweitert (13 Regex-Stellen) — heilt u. a. **zwei vorher STILL GEDROPPTE Absätze VSTG art_5 1sexies/1septies** (Kapitaleinlagen/Kapitalband!), BPV 116m 3sexies/3septies-Labels, HMG art_4 Marker asexies…adecies (5 Zusatz-Erlasse ELG/FINMA_GEBV/HMG/VSTG/VZAE, jede Stelle quell-verifiziert, eigener 4. Adversarial-Pass). §14-Intake: Querschnitt **QS-CURRENCY** im ROADMAP-Querschnitt-Band. Beleg `bibliothek/register/fedlex-currency-2026-07-03.md`. **Offen: P1-c** (56 künftige Fassungen → Wiedervorlage-Generator/Verfallsregister) **+ P1-d** (Currency-Chips + `currency.json`); **Turso-Re-Sync nötig** (Haupt-Session koordiniert).
