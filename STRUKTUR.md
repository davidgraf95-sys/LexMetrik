...
## Session 16.7.2026 — IA-1 «Named-Article-Garantie + Treffer-Dedup + Ergebnis-Kopfzeile» (Such-Engine, Worktree `lm-ia1`, Branch `feat/ia1-named-article`, PR #264)

**Auftrag (FAHRPLAN-GESETZES-UX.md §11.5 IA-1, ROADMAP W2·5d; Risiko-Pfad Such-Engine ⇒ Gegenprüfung Pflicht KC2).** Diff eng auf die Query-Engine + Suche-UI-Verdrahtung — Korpus/`/api/suche` NICHT berührt (koordiniert mit W2·6 PR-B/PR-C). **(1) `src/lib/suche/normQuery.ts`:** ALIAS-Tabelle um 6 amtliche FR/IT-Kurztitel erweitert (CPP→StPO, Cst/Cost→BV, LDIP→IPRG, LTF→BGG, CPM→MStG, je SR-verankert, Quelle Fedlex-Titelblatt); NEU **SR-Nummer-Sprung** «SR 220»/«SR 312.0 Art. 5» über das `sr`-Feld (nur Bund — SR bundesweit eindeutig, 0 Register-Kollisionen; exakte + kollisionsfrei «.0»-gekürzte Form). Zielartikel bleibt oberster `sprung`-Treffer, Enter springt auf `#art-` (NIE `#par-`, R8/K2). **(2) `src/lib/universalSuche.ts` `dedupErlasse`** (nur in der Suche, NICHT im /gesetze-Browse): kollabiert titelgleiche Gemeinde-Doppel (RiE/BeE/BaB-Kopien desselben BS-Erlasses) deterministisch auf die kantonale Kern-Nummer (Schlüssel ebene|kanton|titel → nie kanton-/bund-übergreifend; Riehen-Doppel geheilt). **(3) `SuchResultate.tsx`:** sichtbare Ergebnis-Kopfzeile «n Treffer, davon x Erlasse / y Artikel» (praxis #10; sr-only Live-Region bleibt, sichtbare Zeile aria-hidden).

**Beweise:** neue `e2e/gesetze-ia-v2-walks.e2e.ts` (Interaktions-Zähl-Walks §11.3-Zeilen + Named-Article-Klick-Beweis «OR 336c»/«OR 257d» Budget 1 mit Zielartikel im Viewport + Negativ-Fall «OR 336z» → Liste statt Leerseite; 10/10 grün); Regressions-e2e `norm-sprung`/`suche-seite`/`a11y`/`tastatur`/`gesetze-uebersicht-u`/`-kanton-g5`/`-rechtsgebiet-g6` (50+ grün); `npx tsc -b` + `npm test` (238 Dateien / 3819) + `npm run golden:vergleich` **209 byte-gleich** + lint grün; build 63 Routen. **Gegenprüfung BESTANDEN** (Risiko-Pfad): eigene Re-Derivation 30/30 (10 Zitate × DE/FR/IT gegen echtes Manifest) + unabhängiger Opus-Prüfer gegen Fedlex-SPARQL (Alias-Zuordnung SR-verankert, LTF=BGG/CPM=MStG-Fallen entkräftet; SR Bund-only 0 Kollisionen; 12 Dedup-Gruppen alle echt-identisch; Anker immer `#art-`). Hinweis: `check:gegenpruefung`-Tor klassifiziert `src/lib/suche` NICHT als Risiko-Datei (grün ohne Quittung) → Nachweis über Trailer + diese Karte. **Fremd-rot** (unberührt vom Diff): `check:materialien` (Vernehmlassungs-Stichdaten VERN-2025-124/58) + `check:bibliothek` (S7.3-Link `werkzeuge/omnilex-…` aus PR #251). Trailer `Roadmap: W2·5d` + `Gegenpruefung: bestanden`. §11-Ausführungsvermerk additiv gesetzt (IA-1 ✅). PR mit `--auto --squash`. **Nächster Schritt:** IA-2 (Erfassungsgrad-UI) läuft parallel; danach IA-3.
