...
## Session 16.7.2026 — E3/A34: Split-View des Gesetz-Lesers — Scroll-Rücksprung + fehlendes Ansicht-Menü (W2·5d, reines UI, Worktree `lm-a34`, Branch `fix/a34-splitview`)

**Auftrag (FAHRPLAN-GESETZES-UX.md §10.10 E3, Wortlaut `docs/ux-audit-2026-07/ANMERKUNGEN-DAVID-2026-07-16.md` Ziff. 6).** Zwei Split-View-Bugs, beide empirisch mit Playwright reproduziert (eigener Port 4399 — der geteilte Default-Port 4317 lief mit dem stale Preview-Server eines Parallel-Worktrees `lm-a30`, §12; erst der eigene Port zeigte die echte Wirkung).

**Bug 1 — Scroll-Rücksprung.** Root-Cause `src/pages/gesetz-leser/inhalt.tsx:611` (Seed-Hash-Sprung-Effekt): Deps `[…, imPane, wurzel]` → beim Kippen Einzelansicht→Split-View (imPane false→true) feuerte der Effekt ERNEUT und las `window.location.hash` = der zuvor angeklickte Artikel ⇒ das frisch weitergescrollte Gesetz-Pane sprang dorthin zurück (empirisch: Art. 1 an der Lese-Bezugslinie `top1≈56`, Art. 684 abgeschoben). **Fix:** ein `hashSeedGetan`-Ref (Reset pro `[ebene, schluessel]`) sät den Sprung nur beim ERSTEN inhaltsbereiten Lauf; der Re-Trigger beim Pane-Öffnen kehrt still zurück (spätere Hash-Wechsel trägt ohnehin der `letzteNavKey`-Effekt bzw. die Pane-History). Nach Fix bleibt der übergebene Scroll erhalten (`|scrollTop−scrollVor|<400`), Art. 1 weit oberhalb.

**Bug 2 — Ansicht-Menü fehlt.** Root-Cause `inhalt.tsx` (A26-Erbe): im Split-View lag das «Ansicht»-Dropdown im `ErlassLeserKopf`, der mit dem Normtext WEGSCROLLT — beim Lesen (`scrollTop=4000`) sass der Trigger bei `y≈−3586`, ausserhalb des Pane-Ausschnitts ⇒ «keine Möglichkeit mehr, die Ansicht zu ändern». **Fix:** das imPane-Menü (`ansichtMenuPane`) wandert in die pane-lokalen STICKY Leisten — Such-/Gliederungs-Bar (`!zweiSpalten`) bzw. 2-Spalten-TOC-Sub-Bar (`zweiSpalten`), die sich ausschliessen (nie zwei Menüs); im ErlassLeserKopf nicht mehr gerendert. Kein neues Sticky-Element/kein Stacking- oder CLS-Risiko (beide Leisten existierten sticky). Einzelansicht unberührt (dort trägt der Inhalts-Kopf das Menü, A26).

**Beweise:** neue e2e `e2e/split-view-a34.e2e.ts` (Bug1 Leseposition-Erhalt + kein Rücksprung; Bug2 Menü beim Scrollen im Pane-Ausschnitt + schaltet `data-fussnoten`; beide `test.slow()` gegen CI-CPU-Starvation), lokal 19/19 grün auf eigenem Port. `npm run gate` (voll): tsc · vitest · **golden:vergleich byte-gleich** (Reader-Chrome ausserhalb Golden-Scope) · lint · check ALLE grün, **einzige lokale Röte `check:materialien`** = pre-existing, datumsgetrieben (Vernehmlassungen VERN-2025-124/-58 `fristEnde` 15.7. < 16.7.) — TABU-Datenfläche, nicht A34. CLS-neutral. **Gegenprüfung n/a** (reines UI, kein Rechnen/Extraktion/Norm-Tarif). Trailer `Roadmap: W2·5d`. PR #240 mit armiertem Auto-Merge.
