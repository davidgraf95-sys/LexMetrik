...
## Session 16.7.2026 — E2/A30+A31 «Marginalien-Suffix hochgestellt + Fussnoten-Marker im Fliesstext/ohne Abstand» (Worktree `lm-a30`, Branch `feat/a30-a31-marginalien-fussnoten`)

**Auftrag (`ANMERKUNGEN-DAVID-2026-07-16.md` Ziff. 2+3, `FAHRPLAN-GESETZES-UX.md` §10.10 E2; Referenz = Fedlex-Filestore-HTML, empirisch am gepinnten ZGB-HTML verifiziert §7).** **A30** — Marginalien-/Randtitel-Enumeratoren tragen lat. Ordnungs-Suffixe; Fedlex setzt das **Ordnungs-WORT** (bis/ter/quater…) **hochgestellt** («III<sup>bis</sup>.»), einen **einzelnen Buchstaben-Suffix** («Ia») **kursiv** («I<i>a</i>.»). Neuer reiner Render-Helper `margLabel` (`helpers.tsx`): rekonstruiert die Auszeichnung des schon im String liegenden Suffixes, greift NUR bei röm./arab. Enumerator + Suffix am Wort-/Satzende (No-op bei Sachtiteln wie «Mitgliederverzeichnis»); angewandt in `ArtikelLeser` (Marg-Zeilen), `SektionKopf` (Randtitel), `SektionBaumTOC` (Gliederung). **A31(b)** — Fussnoten-Marker klebte mit `ml-0.5` **und** flex-`gap-x-2` an der Artikelnummer; jetzt sitzt «Art. N» + Marker als EIN `whitespace-nowrap`-Inline-Element (kein Abstand) — Fedlex-treu (Marker als `<sup>` direkt am Bezugswort). Absatz-/Item-/Marginalien-Marker: `ml-0.5` entfernt, via **Wort-Verbinder U+2060** (`wortverbinder.ts`) direkt ans Bezugswort geklebt. **A31(c)** — «grosser Abstand zum nächsten Absatz» = der Marker brach auf schmalem Viewport allein auf eine eigene (voll-hohe Strut-)Zeile um; empirisch (Playwright, Höhenmessung mit/ohne Marker über 6 Breiten): mit WJ bricht **Abs. 1 auf KEINER Breite mehr um** (David-Fall behoben), nur Abs. 2 @390 für das Extrem-Kompositum «Kindesschutzmassnahmen.» (inhärent, Fedlex-Fliesstext ebenso — Marker bleibt funktional). **Bewusst als eigene Bau-Einheit ausgelagert (§14.2, andere Risiko-Klasse): A31(a)** «SR-Fussnote 667 in ZGB 798a gehört INLINE in den Fliesstext statt auf den Artikelkopf» — Root-Cause im **Extraktor** (`fussnoten-extrahiere.ts`: Marker in absatzloser `<p class="absatz">` bekommt `absatz=null`, ununterscheidbar vom Kopf-Marker → Reader legt ihn auf Artikelebene); Fix = Risiko-Pfad (Extraktor + Struktur-Sidecar-Regeneration + Gegenprüfung), separater PR. **Schicht:** A30/A31(b/c) rein Rendering — WJ/margLabel bewusst in `src/components/normtext/` (NICHT `lib/normtext` = Risiko-Pfad; NICHT als Nicht-Komponenten-Export in `ArtikelBody.tsx` = react-refresh). **Beweise:** golden `vergleich` **209 byte-gleich**; tsc/lint (0 Errors) grün; `check:gegenpruefung` grün (**keine Risiko-Datei berührt → n/a**); 35 Reader-e2e grün + 2 neue Asserts (`gesetze-marginalie.e2e.ts`: bis-`<sup>` + nowrap-Klebung); Vorher/Nachher-Screenshots (798a/276/19d, Desktop+Mobil@390, hell+dunkel); kein H-Overflow @390. **Gate ROT nur bei `check:materialien`** (2 Vernehmlassungen fristEnde 2026-07-15 < heute — reiner Datum-Drift auf main, **NICHT in CI-Required**, unabhängig von diesem Diff). Trailer `Roadmap: W2·5d` · `Gegenpruefung: n/a — reine Darstellung`. PR mit armiertem Auto-Merge.
