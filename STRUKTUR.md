...
## Session 17.7.2026 — E7/A33 «Gliederung springt umher» (Ultracode-Fix, Worktree `lm-a33`, Branch `fix/a33-gliederung`)

**Auftrag (FAHRPLAN-GESETZES-UX §10.10 E7 = David-Anmerkung A33 16.7.: «Gliederung springt umher. Wenn man sich darin bewegt.»).** Reine Darstellungsschicht (§3, kein Engine-/Golden-Bezug). Ultracode-verifizierte Root-Causes: **RC1** — der TOC-Mitscroll-Effekt (`inhalt.tsx`) zentrierte animiert auf die Container-MITTE (`- cr.height/2`, `smooth`) statt zu nudgen, ohne Nutzer-Sperre → Halb-Container-Sprünge **289–315 px**, Zurückreissen manuellen Blätterns, Klickziel-Hazard. **RC2** — das Scroll-Spy-Auto-Akkordeon reflowte den Baum unentprellt bei jedem Zweigwechsel. **Fixes:** **F1** Rand-NUDGE statt Zentrieren, **instant** statt smooth (Dead-Band == Zielposition, kein Re-Trigger; kein `scrollIntoView` → Pane-Scope gewahrt); **F2** Nutzer-Interaktions-Guard (passive `wheel`/`pointerdown`/`touchstart` am `[data-toc]`, pane-gescopt → 1,5 s Pause des Nachführens); **F3** Auto-Akkordeon entprellt (~200 ms trailing, analog aktArtikel/tabArtikel; Klick-Sprung bleibt sofort + löscht den Timer); **V1** stille Wiederaufnahme erst beim nächsten echten Artikelwechsel (Effekt keyt auf aktivIds). Verhalten (Auto-Auf/Zuklappen, Auftrag K 26.6.) bleibt — nur Frequenz sinkt; Akkordeon bewusst un-animiert (nichtBauen-3, §15.2). **Beweise:** neuer e2e-Wächter `e2e/leser-gliederung-a33.e2e.ts` (4 Tests grün) — F1 max TOC-Eigenbewegung/Schritt **0 px** (vorher 289–315), F2/V1 Blätter-Position gehalten (Δ<24 px), Klick-Ruhe, A9-CLS unter 4× Drossel <0.05; **CLS-Neutralität belegt** (Baseline 0.1055 vs. Fix 0.1038 unter Synthetik-Schnellscroll = pre-existing Akkordeon-`<li>`-Reflow, nicht F3). 10 bestehende leser-e2e (position-u/kopf-a9/optionen) grün = keine Regression. `npm run gate` GRÜN ausser **check:materialien** (2 Vernehmlassungen VERN-2025-124/58 `fristEnde` 2026-07-15 < heute = fremde Datums-Drift in `vernehmlassungen.generated.ts`, nicht im Diff). golden byte-gleich. Gegenprüfung n/a (reines UI). Trailer `Roadmap: W2·5d`. §10.10-E7-Vermerk additiv. PR mit armiertem Auto-Merge.
